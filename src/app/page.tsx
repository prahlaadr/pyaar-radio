...
function parseUrlParams(): { filters: Partial<ArtistFilters>; artist: string | null; tab: "browse" | "setlists" | "liked" | "albums" | "nts" | "sc" | null; track: string | null; autoplay: boolean; tamil: boolean; ilaiyaraaja: boolean; section: SectionMode; view: "artists" | "tracks"; tvChannel: string | null; tvVideo: string | null; tvTime: number } {
  ...
  const ntssub = p.get("ntssub");
  if (ntssub) filters.ntsSubgenres = ntssub.split(",").filter(Boolean);
...
function buildUrlParams(filters: ArtistFilters, artistName: string | null, tab: "browse" | "setlists" | "liked" | "albums" | "nts" | "sc", trackVideoId?: string | null, tamil?: boolean, browseView?: "artists" | "tracks", section?: SectionMode, ilaiyaraaja?: boolean, tvChannelId?: string | null, tvVideoId?: string | null, tvTimeOffset?: number): string {
  ...
  if (filters.ntsSubgenres.length > 0) p.set("ntssub", filters.ntsSubgenres.join(","));
...
export default function Home() {
  ...
  useEffect(() => {
    if ((filters.ntsGenres.length > 0 || filters.ntsSubgenres.length > 0) && browseView === "artists" && !selectedArtist && !tamilMode && sectionMode === "browse") {
      setBrowseView("tracks");
    }
  }, [filters.ntsGenres, filters.ntsSubgenres, browseView, selectedArtist, tamilMode, sectionMode]);
  ...
  useEffect(() => {
    ...
    const sql = buildFilteredTracksQuery(radioArtists, filters.bpmMin, filters.bpmMax, filters.halfTime, filters.ntsGenres, filters.ntsSubgenres);
    (async () => {
      try {
        const rows = await query<{
          trackName: string; artistNames: string; albumName: string;
  ...
  }, [browseView, artists, filters.bpmMin, filters.bpmMax, filters.halfTime, filters.ntsGenres, filters.ntsSubgenres, selectedArtist, tamilMode, sectionMode]);
  ...
  const fetchLikedTracks = useCallback(async () => {
    // NTS filter active → query the masterlist (which carries the enriched
    // Genres); liked.csv has no genre column so it can't filter by NTS.
    const ng = filters.ntsGenres, ns = filters.ntsSubgenres;
    if (ng.length > 0 || ns.length > 0) {
      try {
        const rows = await query<{
          trackName: string; artistNames: string; albumName: string; genres: string | null;
          tempo: number | null; duration: string; key: number | null; videoId: string;
          soundcloudId: string | null; likedPosition: number | null; firstLikedAt: string | null;
          energy: number | null; danceability: number | null; valence: number | null;
        }>(buildLikedTracksQuery("", ng, ns));
        setLikedTracks(rows.map((r) => ({
          trackName: r.trackName, artistNames: r.artistNames, albumName: r.albumName || "",
          genres: r.genres ? r.genres.split(",").map((g) => g.trim()) : [],
          tempo: Number(r.tempo) || 0, duration: r.duration || "", key: Number(r.key) || 0, popularity: 0,
          videoId: r.videoId || "", soundcloudId: r.soundcloudId || "", bandcampId: "",
          likedPosition: r.likedPosition ?? null, firstLikedAt: r.firstLikedAt ?? null,
          energy: r.energy ?? null, danceability: r.danceability ?? null, valence: r.valence ?? null,
        })));
  ...
  }, [filters.ntsGenres, filters.ntsSubgenres]);

  useEffect(() => {
    if (tab === "liked") fetchLikedTracks();
  }, [tab, fetchLikedTracks]);
