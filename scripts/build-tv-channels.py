#!/usr/bin/env python3
"""Build channels.json for pyaar.tv using yt-dlp."""

import json
import subprocess
import sys

OUT = "public/data/tv/channels.json"

# (id, name, number, color, yt_source, max_videos)
CHANNELS = [
    ("dj-sets", "DJ Sets", 1, "#ef4444", "https://www.youtube.com/@boilerroom/videos", 8),
    ("live-performances", "Live Performances", 2, "#f59e0b", "https://www.youtube.com/@nprmusic/search?query=tiny%20desk%20concert", 8),
    ("music-docs", "Music Docs", 3, "#8b5cf6", "https://www.youtube.com/@Polyphonic/videos", 6),
    ("ilaiyaraaja", "Ilaiyaraaja", 4, "#dc2626", "ytsearch8:ilaiyaraaja full songs concert film", 8),
    ("ramana-balachandran", "Ramana Balachandran", 5, "#f97316", "ytsearch6:ramana balachandran carnatic violin", 6),
    ("carnatic", "Carnatic", 6, "#eab308", "ytsearch8:carnatic concert full TM Krishna Sanjay Subrahmanyan", 8),
    ("nts-radio", "NTS Radio", 7, "#06b6d4", "https://www.youtube.com/@TheLotRadio/videos", 6),
    ("jeopardy", "Jeopardy", 8, "#3b82f6", "ytsearch6:jeopardy full episode compilation", 6),
    ("tom-and-jerry", "Tom & Jerry", 9, "#a855f7", "ytsearch5:tom and jerry classic compilation full", 5),
    ("bts", "BTS", 10, "#ec4899", "ytsearch10:BTS official music video", 10),
    ("top-gear", "Top Gear", 11, "#22c55e", "ytsearch5:top gear special full episode Bolivia Vietnam", 5),
    ("all-gas-no-brakes", "Channel 5", 12, "#14b8a6", "https://www.youtube.com/@Channel5YouTube/videos", 8),
    ("good-mythical-morning", "Good Mythical Morning", 13, "#84cc16", "https://www.youtube.com/@GoodMythicalMorning/videos", 8),
    ("tiny-desk", "Tiny Desk", 14, "#f43f5e", "https://www.youtube.com/@nprmusic/videos", 10),
    ("colors", "COLORS", 15, "#fb923c", "https://www.youtube.com/@COLORSxSTUDIOS/videos", 10),
    ("like-a-version", "Like a Version", 16, "#facc15", "ytsearch8:triple j like a version best", 8),
    ("boiler-room", "Boiler Room", 17, "#171717", "https://www.youtube.com/@boilerroom/videos", 8),
    ("madrasana", "MadRasana", 18, "#b91c1c", "https://www.youtube.com/@MadRasana/videos", 8),
    ("coke-studio", "Coke Studio", 19, "#e11d48", "ytsearch8:coke studio season 14 15 best", 8),
    ("lot-radio", "The Lot Radio", 20, "#737373", "https://www.youtube.com/@TheLotRadio/videos", 5),
    ("hot-ones", "Hot Ones", 21, "#ea580c", "https://www.youtube.com/@FirstWeFeast/videos", 8),
    ("nardwuar", "Nardwuar", 22, "#e879f9", "https://www.youtube.com/@nardwuar/videos", 8),
    ("kill-tony", "Kill Tony", 23, "#991b1b", "https://www.youtube.com/@KillTony/videos", 5),
    ("rdcworld", "RDCworld1", 24, "#0ea5e9", "https://www.youtube.com/@RDCworld1/videos", 8),
    ("team-coco", "Team Coco", 25, "#f97316", "https://www.youtube.com/@TeamCoco/videos", 6),
    ("qi", "QI", 26, "#2563eb", "https://www.youtube.com/@TheQIElves/videos", 8),
    ("graham-norton", "Graham Norton", 27, "#7c3aed", "https://www.youtube.com/@OfficialGrahamNorton/videos", 6),
    ("snl", "SNL", 28, "#f5f5f4", "https://www.youtube.com/@SaturdayNightLive/videos", 8),
    ("kurzgesagt", "Kurzgesagt", 29, "#38bdf8", "https://www.youtube.com/@kurzgesagt/videos", 8),
    ("vox", "Vox", 30, "#fbbf24", "https://www.youtube.com/@Vox/videos", 8),
    ("defunctland", "Defunctland", 31, "#065f46", "https://www.youtube.com/@Defunctland/videos", 6),
    ("johnny-harris", "Johnny Harris", 32, "#1d4ed8", "https://www.youtube.com/@johnnyharris/videos", 6),
    ("fireship", "Fireship", 33, "#f97316", "https://www.youtube.com/@Fireship/videos", 10),
    ("mkbhd", "MKBHD", 34, "#dc2626", "https://www.youtube.com/@mkbhd/videos", 6),
    ("doug-demuro", "Doug DeMuro", 35, "#16a34a", "https://www.youtube.com/@DougDeMuro/videos", 6),
    ("architectural-digest", "Architectural Digest", 36, "#a3a3a3", "https://www.youtube.com/@Archdigest/videos", 6),
    ("spongebob", "SpongeBob", 37, "#facc15", "ytsearch4:spongebob full episodes compilation marathon", 4),
    ("nba", "NBA Highlights", 38, "#1d4ed8", "ytsearch6:nba highlights 2025 full game best plays", 6),
    ("needledrop", "theneedledrop", 39, "#a3e635", "https://www.youtube.com/@theneedledrop/videos", 8),
]


def fetch_videos(source: str, max_videos: int) -> list[dict]:
    """Fetch videos from a YouTube source using yt-dlp."""
    try:
        result = subprocess.run(
            [
                "yt-dlp", "--flat-playlist",
                "--print", "%(id)s\t%(title)s\t%(duration)s",
                "--playlist-end", str(max_videos),
                source,
            ],
            capture_output=True, text=True, timeout=60,
        )
        videos = []
        for line in result.stdout.strip().split("\n"):
            if not line.strip():
                continue
            parts = line.split("\t")
            if len(parts) < 3:
                continue
            vid_id, title, duration_str = parts[0], parts[1], parts[2]
            try:
                duration = int(float(duration_str))
            except (ValueError, TypeError):
                continue
            if duration <= 0 or not vid_id:
                continue
            videos.append({
                "videoId": vid_id,
                "title": title,
                "durationSeconds": duration,
            })
        return videos
    except subprocess.TimeoutExpired:
        return []
    except Exception as e:
        print(f"    ⚠ Error: {e}", file=sys.stderr)
        return []


def main():
    channels = []
    total_videos = 0

    for ch_id, name, number, color, source, max_vids in CHANNELS:
        print(f"  [{number:>2}] {name} — fetching {max_vids} videos...")
        videos = fetch_videos(source, max_vids)
        if not videos:
            print(f"       ⚠ No videos found, using empty list")
        else:
            print(f"       ✓ {len(videos)} videos")
            total_videos += len(videos)

        channels.append({
            "id": ch_id,
            "name": name,
            "number": number,
            "color": color,
            "videos": videos,
        })

    data = {"channels": channels}

    with open(OUT, "w") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)

    print(f"\n✓ Done! {len(channels)} channels, {total_videos} videos")
    print(f"  Written to: {OUT}")


if __name__ == "__main__":
    main()
